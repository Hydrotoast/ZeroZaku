...
onload_functions[onload_functions.length] = 'ca_set_logo();';
...
function ca_popup_start_hide(id)
{
    ...
    setTimeout("ca_popup_end_hide('" + id + "')", 250);
}
...
// set logo
function ca_set_logo()
{
    ca_item = document.getElementById('logoimg');
    if(ca_item)
    {
        ca_list = ca_item.getElementsByTagName('img');
        if(ca_list.length > 0)
        {
            ca_item2 = document.getElementById('logolink');
            if(ca_item2)
            {
                ca_item2.style.backgroundImage = 'url(' + ca_list[0].src + ')';
                if(ca_list[0].width > 0)
                {
                    ca_item2.style.width = ca_list[0].width + 'px';
                }
                if(ca_list[0].height > 0)
                {
                    ca_item2.style.height = ca_list[0].height + 'px';
                    ca_item2 = document.getElementById('logo');
                    if(ca_item2)
                    {
                        ca_item2.style.height = ca_list[0].height + 'px';
                    }
