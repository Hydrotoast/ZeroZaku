...
if ($config['allow_bookmarks'] && $user->data['is_registered'] && request_var('bookmark', 0))
{
	if (check_link_hash(request_var('hash', ''), "topic_$topic_id"))
	{
		...
		$message = (($topic_data['bookmarked']) ? $user->lang['BOOKMARK_REMOVED'] : $user->lang['BOOKMARK_ADDED']) . '<br />' . sprintf($user->lang['RETURN_TOPIC'], '<a href="' . $viewtopic_url . '">', '</a>');
	}
	else
	{
		$message = $user->lang['BOOKMARK_ERR'] . '<br />' . sprintf($user->lang['RETURN_TOPIC'], '<a href="' . $viewtopic_url . '">', '</a>');
	}
...
if (!empty($topic_data['poll_start']))
{
	...
	if ($update && $s_can_vote)
	{
		...
		if (!sizeof($voted_id) || sizeof($voted_id) > $topic_data['poll_max_options'] || in_array(VOTE_CONVERTED, $cur_voted_id) || !check_form_key('posting'))
		{
			...
			$message = $user->lang[$message] . '<br />' . sprintf($user->lang['RETURN_TOPIC'], '<a href="' . $redirect_url . '">', '</a>');
			trigger_error($message);
		}
		...
		meta_refresh(5, $redirect_url);
		trigger_error($user->lang['VOTE_SUBMITTED'] . '<br />' . sprintf($user->lang['RETURN_TOPIC'], '<a href="' . $redirect_url . '">', '</a>'));
	}
...
$sql = $db->sql_build_query('SELECT', array(
	'SELECT'	=> 'u.*, z.friend, z.foe, p.*, pb.id AS pb_id, pb.holding AS pb_holding, c.message',
...
for ($i = 0, $end = sizeof($post_list); $i < $end; ++$i)
{
	...
	$postrow = array(
		...
		'S_TOPIC_AUTHOR'        => ($poster_id == $topic_data['topic_poster']) ? true : false,
		...
		'POST_ID'			=> $row['post_id'],
		'POSTER_ID'			=> $poster_id,
		'QUICK_COMMENT'		=> append_sid("{$phpbb_root_path}comment.$phpEx", "u=$poster_id"),

		'S_IS_BANNED' 		=> (in_array($poster_id, $ban_list)) ? true : false,
