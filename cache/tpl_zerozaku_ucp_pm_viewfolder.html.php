...
<?php } else { if ($this->_rootref['NUM_REMOVED']) {  ?>
	...
	<?php } if (sizeof($this->_tpldata['messagerow'])) {  ?>
		...
		<ul class="topiclist">
			<li class="header">
				<dl class="meta">
					<dt class="message"><div class="inner"><?php echo ((isset($this->_rootref['L_MESSAGE'])) ? $this->_rootref['L_MESSAGE'] : ((isset($user->lang['MESSAGE'])) ? $user->lang['MESSAGE'] : '{ MESSAGE }')); ?></div></dt>
		...
		<?php $_messagerow_count = (isset($this->_tpldata['messagerow'])) ? sizeof($this->_tpldata['messagerow']) : 0;if ($_messagerow_count) {for ($_messagerow_i = 0; $_messagerow_i < $_messagerow_count; ++$_messagerow_i){$_messagerow_val = &$this->_tpldata['messagerow'][$_messagerow_i]; ?>
			...
			<li class="row<?php if (($_messagerow_val['S_ROW_COUNT'] & 1)  ) {  ?> bg1<?php } else { ?> bg2<?php } if ($_messagerow_val['PM_CLASS']) {  ?> <?php echo $_messagerow_val['PM_CLASS']; } ?>">
				<dl class="icon" style="background-image: url(<?php echo $_messagerow_val['FOLDER_IMG_SRC']; ?>); background-repeat: no-repeat; background-position: 10px center;">
					<dt>
						<div class="inner">
							...
							<?php } else { ?>
								...
								<a href="<?php echo $_messagerow_val['U_VIEW_PM']; ?>" class="topictitle"><?php echo $_messagerow_val['SUBJECT']; ?> <?php if ($_messagerow_val['PM_ICON_URL'] && $this->_rootref['S_PM_ICONS']) {  ?><img src="<?php echo $_messagerow_val['PM_ICON_URL']; ?>" alt="<?php echo $_messagerow_val['SUBJECT']; ?>" /><?php } ?></a>
