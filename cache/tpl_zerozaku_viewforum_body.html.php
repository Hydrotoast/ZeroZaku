<?php if (!defined('IN_PHPBB')) exit; $this->_tpldata['DEFINE']['.']['CA_PAGE'] = 'viewforum'; $this->_tpl_include('overall_header.html'); if ($this->_rootref['FORUM_DESC'] || $this->_rootref['FORUM_SEO_DESC']) {  ?>
	...
	<div class="panel smooth info <?php if ($this->_rootref['FORUM_SEO_DESC']) {  ?>seo<?php } ?>">
			<?php if ($this->_rootref['FORUM_SEO_DESC']) {  ?><img src="<?php echo (isset($this->_rootref['T_THEME_PATH'])) ? $this->_rootref['T_THEME_PATH'] : ''; ?>/images/tag_blue.png" alt="<?php echo (isset($this->_rootref['FORUM_SEO_KEY'])) ? $this->_rootref['FORUM_SEO_KEY'] : ''; ?>" /><p><strong><?php echo (isset($this->_rootref['FORUM_SEO_KEY'])) ? $this->_rootref['FORUM_SEO_KEY'] : ''; ?></strong><br /><span><?php echo (isset($this->_rootref['FORUM_SEO_DESC'])) ? $this->_rootref['FORUM_SEO_DESC'] : ''; ?><br /></span></p><?php } else if ($this->_rootref['FORUM_DESC']) {  ?><img src="<?php echo (isset($this->_rootref['T_THEME_PATH'])) ? $this->_rootref['T_THEME_PATH'] : ''; ?>/images/info.png" alt="Forum Description" /><p><?php echo (isset($this->_rootref['FORUM_DESC'])) ? $this->_rootref['FORUM_DESC'] : ''; ?></p><?php } ?>

	</div>
<?php } if ($this->_rootref['MODERATORS']) {  ?><p><strong><?php if ($this->_rootref['S_SINGLE_MODERATOR']) {  echo ((isset($this->_rootref['L_MODERATOR'])) ? $this->_rootref['L_MODERATOR'] : ((isset($user->lang['MODERATOR'])) ? $user->lang['MODERATOR'] : '{ MODERATOR }')); } else { echo ((isset($this->_rootref['L_MODERATORS'])) ? $this->_rootref['L_MODERATORS'] : ((isset($user->lang['MODERATORS'])) ? $user->lang['MODERATORS'] : '{ MODERATORS }')); } ?>:</strong> <?php echo (isset($this->_rootref['MODERATORS'])) ? $this->_rootref['MODERATORS'] : ''; ?></p><?php } if ($this->_rootref['S_HAS_SUBFORUM']) {  if (! $this->_rootref['S_IS_BOT'] && $this->_rootref['U_MARK_FORUMS']) {  ?>

<ul class="linklist">
	<li class="rightside"><a class="button2" href="<?php echo (isset($this->_rootref['U_MARK_FORUMS'])) ? $this->_rootref['U_MARK_FORUMS'] : ''; ?>"><?php echo ((isset($this->_rootref['L_MARK_FORUMS_READ'])) ? $this->_rootref['L_MARK_FORUMS_READ'] : ((isset($user->lang['MARK_FORUMS_READ'])) ? $user->lang['MARK_FORUMS_READ'] : '{ MARK_FORUMS_READ }')); ?></a></li>
</ul>
	...
	<li class="row<?php if (!($_topicrow_val['S_ROW_COUNT'] & 1)  ) {  ?> bg1<?php } else { ?> bg2<?php } if ($_topicrow_val['S_POST_ANNOUNCE']) {  ?> announce<?php } if ($_topicrow_val['S_POST_STICKY']) {  ?> sticky<?php } if ($_topicrow_val['S_TOPIC_REPORTED'] || $_topicrow_val['S_TOPIC_DELETED']) {  ?> reported<?php } ?>">
		<dl class="icon">
			<dt<?php if ($_topicrow_val['TOPIC_ICON_IMG'] && $this->_rootref['S_TOPIC_ICONS']) {  ?> style="background-image: url(<?php echo (isset($this->_rootref['T_ICONS_PATH'])) ? $this->_rootref['T_ICONS_PATH'] : ''; echo $_topicrow_val['TOPIC_ICON_IMG']; ?>); background-repeat: no-repeat;"<?php } else { ?> style="background-image: url(<?php echo $_topicrow_val['TOPIC_FOLDER_IMG_SRC']; ?>); background-repeat: no-repeat; background-position: 10px center"<?php } ?> title="<?php echo $_topicrow_val['TOPIC_FOLDER_IMG_ALT']; ?>">
				...
				<a href="<?php echo $_topicrow_val['U_VIEW_TOPIC']; ?>" class="topictitle"><span class="meta"><?php if ($_topicrow_val['S_TOPIC_REPORTED']) {  ?>[reported] <?php } ?></span><?php echo $_topicrow_val['TOPIC_TITLE']; ?></a>
