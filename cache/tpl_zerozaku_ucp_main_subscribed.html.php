...
<?php if (sizeof($this->_tpldata['forumrow'])) {  ?>
	...
	<ul class="topiclist">
		<li class="header">
			<dl class="icon">
				<dt><div class="inner"><?php echo ((isset($this->_rootref['L_WATCHED_FORUMS'])) ? $this->_rootref['L_WATCHED_FORUMS'] : ((isset($user->lang['WATCHED_FORUMS'])) ? $user->lang['WATCHED_FORUMS'] : '{ WATCHED_FORUMS }')); ?></div></dt>
				<dd class="lastpost"><div class="inner"><?php echo ((isset($this->_rootref['L_LAST_POST'])) ? $this->_rootref['L_LAST_POST'] : ((isset($user->lang['LAST_POST'])) ? $user->lang['LAST_POST'] : '{ LAST_POST }')); ?></div></dd>
	...
	<?php $_forumrow_count = (isset($this->_tpldata['forumrow'])) ? sizeof($this->_tpldata['forumrow']) : 0;if ($_forumrow_count) {for ($_forumrow_i = 0; $_forumrow_i < $_forumrow_count; ++$_forumrow_i){$_forumrow_val = &$this->_tpldata['forumrow'][$_forumrow_i]; ?>
		...
		<li class="row<?php if (($_forumrow_val['S_ROW_COUNT'] & 1)  ) {  ?> bg1<?php } else { ?> bg2<?php } ?>">
			<dl class="icon">
				<dt  style="background-image: url(<?php echo $_forumrow_val['FORUM_FOLDER_IMG_SRC']; ?>); background-repeat: no-repeat; background-position: 10px center;"><div class="inner"><a href="<?php echo $_forumrow_val['U_VIEWFORUM']; ?>" class="forumtitle"><?php echo $_forumrow_val['FORUM_NAME']; ?></a><br /><?php echo $_forumrow_val['FORUM_DESC']; ?></div></dt>
				...
				<dd class="mark"><div class="inner"><input type="checkbox" name="f[<?php echo $_forumrow_val['FORUM_ID']; ?>]" id="f<?php echo $_forumrow_val['FORUM_ID']; ?>" /></div></dd>
			</dl>
		</li>
	<?php }} ?>
