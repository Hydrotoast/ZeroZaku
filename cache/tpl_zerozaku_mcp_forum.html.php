...
<div class="panel">
	...
	<?php } if (sizeof($this->_tpldata['topicrow'])) {  ?>
		...
		<ul class="topiclist">
			<li class="header">
				<dl class="meta">
					<dt class="forum"><div class="inner"><?php echo ((isset($this->_rootref['L_TOPICS'])) ? $this->_rootref['L_TOPICS'] : ((isset($user->lang['TOPICS'])) ? $user->lang['TOPICS'] : '{ TOPICS }')); ?></div></dt>
					<dd class="posts"><div class="inner"><?php echo ((isset($this->_rootref['L_REPLIES'])) ? $this->_rootref['L_REPLIES'] : ((isset($user->lang['REPLIES'])) ? $user->lang['REPLIES'] : '{ REPLIES }')); ?></div></dd>
		...
		<li class="row<?php if (($_topicrow_val['S_ROW_COUNT'] & 1)  ) {  ?> bg1<?php } else { ?> bg2<?php } if ($_topicrow_val['S_TOPIC_REPORTED'] || $_topicrow_val['S_TOPIC_DELETED']) {  ?> reported<?php } ?>">
			<dl class="icon" style="background-image: url(<?php echo $_topicrow_val['TOPIC_FOLDER_IMG_SRC']; ?>); background-repeat: no-repeat; background-position: 10px center;">
