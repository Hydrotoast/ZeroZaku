<?php if (!defined('IN_PHPBB')) exit; $this->_tpldata['DEFINE']['.']['CA_PAGE'] = 'memberlist'; if ($this->_rootref['S_IN_SEARCH_POPUP']) {  $this->_tpl_include('simple_header.html'); $this->_tpl_include('memberlist_search.html'); ?>
...
<?php } if ($this->_rootref['S_SHOW_GROUP']) {  ?>
	...
	<?php } else { ?>
		...
		<h2 class="pagetitle"><?php echo (isset($this->_rootref['PAGE_TITLE'])) ? $this->_rootref['PAGE_TITLE'] : ''; if ($this->_rootref['SEARCH_WORDS']) {  ?>: <a href="<?php echo (isset($this->_rootref['U_SEARCH_WORDS'])) ? $this->_rootref['U_SEARCH_WORDS'] : ''; ?>"><?php echo (isset($this->_rootref['SEARCH_WORDS'])) ? $this->_rootref['SEARCH_WORDS'] : ''; ?></a><?php } ?></h2>
...
<div class="forumbg">
	...
	<tr class="<?php if (!($_memberrow_val['S_ROW_COUNT'] & 1)  ) {  ?>bg1<?php } else { ?>bg2<?php } ?>">
		<td class="name"><?php if ($_memberrow_val['RANK_IMG']) {  ?><span class="rank-img"><?php echo $_memberrow_val['RANK_IMG']; ?></span><?php } else { ?><span class="rank-img"><?php echo $_memberrow_val['RANK_TITLE']; ?></span><?php } if ($this->_rootref['S_IN_SEARCH_POPUP'] && ! $this->_rootref['S_SELECT_SINGLE']) {  ?><input type="checkbox" name="user" value="<?php echo $_memberrow_val['USERNAME']; ?>" /> <?php } echo $_memberrow_val['USERNAME_FULL']; if ($this->_rootref['S_SELECT_SINGLE']) {  ?><br />[&nbsp;<a href="#" onclick="insert_single('<?php echo $_memberrow_val['A_USERNAME']; ?>'); return false;"><?php echo ((isset($this->_rootref['L_SELECT'])) ? $this->_rootref['L_SELECT'] : ((isset($user->lang['SELECT'])) ? $user->lang['SELECT'] : '{ SELECT }')); ?></a>&nbsp;]<?php } ?></td>
		...
		<td class="joined"><?php echo $_memberrow_val['JOINED']; ?></td>
		<?php if ($this->_rootref['S_VIEWONLINE']) {  ?><td class="visited"><?php echo $_memberrow_val['VISITED']; ?>&nbsp;</td><?php } ?>
