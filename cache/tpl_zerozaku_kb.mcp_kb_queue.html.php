...
<div class="panel">
	...
	<p><?php echo ((isset($this->_rootref['L_KB_QUEUE_EXPLAIN'])) ? $this->_rootref['L_KB_QUEUE_EXPLAIN'] : ((isset($user->lang['KB_QUEUE_EXPLAIN'])) ? $user->lang['KB_QUEUE_EXPLAIN'] : '{ KB_QUEUE_EXPLAIN }')); ?></p>
	...
	<?php if (sizeof($this->_tpldata['articlerow'])) {  ?>
		...
		<?php $_articlerow_count = (isset($this->_tpldata['articlerow'])) ? sizeof($this->_tpldata['articlerow']) : 0;if ($_articlerow_count) {for ($_articlerow_i = 0; $_articlerow_i < $_articlerow_count; ++$_articlerow_i){$_articlerow_val = &$this->_tpldata['articlerow'][$_articlerow_i]; ?>
			...
			<li class="row<?php if (!($_articlerow_val['S_ROW_COUNT'] & 1)  ) {  ?> bg1<?php } else { ?> bg2<?php } ?>">
				<dl class="icon">
					<dt style="background-image: url(<?php echo $_articlerow_val['ARTICLE_FOLDER_IMG_SRC']; ?>); background-repeat: no-repeat;">
						<div class="inner">
							<?php if ($_articlerow_val['ARTICLE_TYPE_IMG']) {  ?><img src="<?php echo $_articlerow_val['ARTICLE_TYPE_IMG']; ?>" width="<?php echo $_articlerow_val['ARTICLE_TYPE_IMG_WIDTH']; ?>" height="<?php echo $_articlerow_val['ARTICLE_TYPE_IMG_HEIGHT']; ?>" alt="" /><?php } ?><a href="<?php echo $_articlerow_val['U_VIEW_ARTICLE']; ?>" class="topictitle"><?php echo $_articlerow_val['ARTICLE_TITLE']; ?></a>
							<?php if ($_articlerow_val['ARTICLE_LAST_EDIT']) {  ?><br /><?php echo $_articlerow_val['ARTICLE_LAST_EDIT']; } ?>

						</div>
					</dt>
					<dd class="lastpost"><div class="inner"><span><?php echo ((isset($this->_rootref['L_POST_BY_AUTHOR'])) ? $this->_rootref['L_POST_BY_AUTHOR'] : ((isset($user->lang['POST_BY_AUTHOR'])) ? $user->lang['POST_BY_AUTHOR'] : '{ POST_BY_AUTHOR }')); ?> <?php echo $_articlerow_val['ARTICLE_AUTHOR_FULL']; ?><br /><?php if ($_articlerow_val['ATTACH_ICON_IMG']) {  echo $_articlerow_val['ATTACH_ICON_IMG']; ?> <?php } echo $_articlerow_val['FIRST_POST_TIME']; ?>	</div></span>
					</dd>
					<dd class="mark"><div class="inner"><a href="<?php echo $_articlerow_val['U_STATUS']; ?>" title="<?php echo ((isset($this->_rootref['L_ALTER_STATUS'])) ? $this->_rootref['L_ALTER_STATUS'] : ((isset($user->lang['ALTER_STATUS'])) ? $user->lang['ALTER_STATUS'] : '{ ALTER_STATUS }')); ?>"><?php echo $_articlerow_val['ARTICLE_STATUS']; ?></a></div></dd>
				</dl>
			</li>
		<?php }} ?>
