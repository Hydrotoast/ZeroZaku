...
if ($mode == 'bump')
{
	   ...
	   && check_link_hash(request_var('hash', ''), "topic_{$post_data['topic_id']}"))
	{
		...
		$message = $user->lang['TOPIC_BUMPED'] . '<br />' . sprintf($user->lang['VIEW_MESSAGE'], '<a href="' . $meta_url . '">', '</a>');
...
if ($save && $user->data['is_registered'] && $auth->acl_get('u_savedrafts') && ($mode == 'reply' || $mode == 'post' || $mode == 'quote'))
{
	...
	if ($subject && $message)
	{
		if (confirm_box(true))
		{
			...
			$message = $user->lang['DRAFT_SAVED'] . '<br />';
			$message .= ($mode != 'post') ? sprintf($user->lang['RETURN_TOPIC'], '<a href="' . $meta_info . '">', '</a>') . '<br />' : '';
...
// Begin : Anti Double Posts 
	...
	{		
		...
		//Do the job...
		$message_parser->message = $adp_post_text . '<br />' . $post_data['adp_text_edit'] . '<br />' . $message_parser->message;
	}
// End : Anti Double Posts
	...
	if (!sizeof($error) && $submit)
	{
		...
		if ($submit)
		{
			...
			else
			{
				...
				$message = $user->lang[$message] . '<br />' . sprintf($user->lang['VIEW_MESSAGE'], '<a href="' . $redirect_url . '">', '</a>');
			}

			$message .= '<br />' . sprintf($user->lang['RETURN_FORUM'], '<a href="' . append_sid("{$phpbb_root_path}viewforum.$phpEx", 'f=' . $data['forum_id']) . '">', '</a>');
			trigger_error($message);
		}
	}
}
	...
	if ($auth->acl_get('m_delete', $forum_id) || ($post_data['poster_id'] == $user->data['user_id'] && $user->data['is_registered'] && $auth->acl_get('f_delete', $forum_id) && $post_id == $post_data['topic_last_post_id'] && !$post_data['post_edit_locked'] && ($post_data['post_time'] > time() - ($config['delete_time'] * 60) || !$config['delete_time'])))
	{
		...
		if (confirm_box(true))
		{
			...
			else
			{
				...
				$message = $user->lang['POST_DELETED'] . '<br />' . sprintf($user->lang['RETURN_TOPIC'], '<a href="' . $meta_info . '">', '</a>');
			}
