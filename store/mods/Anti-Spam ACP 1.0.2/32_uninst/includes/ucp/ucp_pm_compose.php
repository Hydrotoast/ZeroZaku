...
function compose_pm($id, $mode, $action)
{
	...
	if ($action == 'delete')
	{
		...
		if (confirm_box(true))
		{
			...
			$message .= '<br />' . sprintf($user->lang['RETURN_FOLDER'], '<a href="' . $meta_info . '">', '</a>');
	...
	if ($save && $auth->acl_get('u_savedrafts'))
	{
		...
		if ($subject && $message)
		{
			if (confirm_box(true))
			{
				...
				$message = $user->lang['DRAFT_SAVED'] . '<br />' . sprintf($user->lang['RETURN_UCP'], '<a href="' . $redirect_url . '">', '</a>');
	...
	if ($submit || $preview || $refresh)
	{
		...
		if (!sizeof($error) && $submit)
		{
			...
			$message = $user->lang['MESSAGE_STORED'] . '<br />' . sprintf($user->lang['VIEW_PRIVATE_MESSAGE'], '<a href="' . $return_message_url . '">', '</a>') . '<br />' . sprintf($user->lang['CLICK_RETURN_FOLDER'], '<a href="' . $return_folder_url . '">', '</a>', $user->lang['PM_OUTBOX']);
			trigger_error($message);
		}
