...
function delete_post($forum_id, $topic_id, $post_id, &$data)
{
	...
	switch ($post_mode)
	{
		...
		case 'delete_first_post':
			$sql = 'SELECT p.post_id, p.poster_id, p.points_received, p.post_username, u.username, u.user_colour
				FROM ' . POSTS_TABLE . ' p, ' . USERS_TABLE . " u
				WHERE p.topic_id = $topic_id
		...
		case 'delete':
			$sql = 'SELECT post_id, poster_id, points_received
				FROM ' . POSTS_TABLE . "
				WHERE topic_id = $topic_id " .
					((!$auth->acl_get('m_approve', $forum_id)) ? 'AND post_approved = 1' : '') . '
...
function submit_post($mode, $subject, $username, $topic_type, &$poll, &$data, $update_message = true)
{
	...
	if (isset($poll['poll_options']) && !empty($poll['poll_options']))
	{
		...
		// Start Ultimate Points
		if ( $mode == 'post' && $config['points_enable'] )
		{
	...
	if (!empty($data['attachment_data']) && $data['post_id'] && in_array($mode, array('post', 'reply', 'quote', 'edit')))
	{
		...
		// Start Ultimate Points
		if ( $mode == 'post' || $mode == 'reply' && $config['points_enable'] == 1 )
		{
