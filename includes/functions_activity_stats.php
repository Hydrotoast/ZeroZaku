...
function activity_mod()
{
	...
	$active_guests = obtain_active_guest_data();
...
function obtain_active_user_data()
{
	...
	if (($active_users = $cache->get('_active_users')) === false)
	{
		...
		$sql_ary = array(
			...
			'WHERE'		=> 'u.user_lastvisit > ' . (time() - 2592000) . ' OR s.session_user_id <> ' . ANONYMOUS,
			'GROUP_BY'	=> 'u.user_id',
			'ORDER_BY'	=> 'u.username',
		);
 ...
 * @return array
 */
function obtain_active_guest_data()
{
	global $cache;

	if (($active_guests = $cache->get('_active_guests')) === false)
	{
		global $db;
		...
		// grab a list of users who are currently online
		// and users who have visited in the last 24 hours
		$sql = 'SELECT session_id FROM ' . SESSIONS_TABLE . ' WHERE session_last_visit > ' . (time() - 2592000) . ' AND session_user_id = ' . ANONYMOUS;
		$result = $db->sql_query($sql);

		while ($row = $db->sql_fetchrow($result))
		{
			$active_guests[$row['session_id']] = array(
		...
		// cache this data for 1 hour, this improves performance
		$cache->put('_active_guests', $active_guests, 3600);
	}
