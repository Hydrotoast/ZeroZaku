	...
	function main($id, $mode)
	{
		...
		if ($submit)
		{
			...
			if ($jab_enable)
			{
				...
				if (!$jabber->connect())
				{
					trigger_error($user->lang['ERR_JAB_CONNECT'] . '<br />' . $jabber->get_log() . adm_back_link($this->u_action), E_USER_WARNING);
				}

				// We'll try to authorise using this account
				if (!$jabber->login())
				{
					trigger_error($user->lang['ERR_JAB_AUTH'] . '<br />' . $jabber->get_log() . adm_back_link($this->u_action), E_USER_WARNING);
				}
