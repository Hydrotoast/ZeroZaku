...
function move_pm($user_id, $message_limit, $move_msg_ids, $dest_folder, $cur_folder_id)
{
		...
		!in_array($cur_folder_id, array(PRIVMSGS_NO_BOX, PRIVMSGS_OUTBOX)) && $cur_folder_id != $dest_folder)
	{
		...
		if ($dest_folder != PRIVMSGS_INBOX)
		{
			...
			{
				$message = sprintf($user->lang['NOT_ENOUGH_SPACE_FOLDER'], $row['folder_name']) . '<br />';
		...
		else
		{
			...
			{
				$message = sprintf($user->lang['NOT_ENOUGH_SPACE_FOLDER'], $user->lang['PM_INBOX']) . '<br />';
	...
	switch ($mark_action)
	{
		...
		case 'delete_marked':
			...
			if (confirm_box(true))
			{
				...
				trigger_error($user->lang[$success_msg] . '<br />' . sprintf($user->lang['RETURN_FOLDER'], '<a href="' . $redirect . '">', '</a>'));
			}
			else
			{
