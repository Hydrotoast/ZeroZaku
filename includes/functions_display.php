...
function display_forums($root_data = '', $display_moderators = true, $return_moderators = false)
{
	...
	if ($mark_read == 'forums' || $mark_read == 'all')
	{
		...
		if (check_link_hash($token, 'global'))
		{
			...
			trigger_error($user->lang['FORUMS_MARKED'] . '<br />' . $message);
		}
		else
		{
		...
		if (!is_null($notify_status) && $notify_status !== '')
		{
			...
			if (isset($_GET['unwatch']))
			{
				...
				if ($uid != $user_id)
				{
					...
					$message = $user->lang['ERR_UNWATCHING'] . '<br />' . sprintf($user->lang['RETURN_' . strtoupper($mode)], '<a href="' . $redirect_url . '">', '</a>');
					trigger_error($message);
				}
				...
				$message = $user->lang['NOT_WATCHING_' . strtoupper($mode)] . '<br />' . sprintf($user->lang['RETURN_' . strtoupper($mode)], '<a href="' . $redirect_url . '">', '</a>');
				trigger_error($message);
			}
			else
			{
		...
		else
		{
			if (isset($_GET['watch']))
			{
				...
				if ($_GET['watch'] == $mode && check_link_hash($token, "{$mode}_$match_id"))
				{
					...
					$db->sql_query($sql);
					$message = $user->lang['ARE_WATCHING_' . strtoupper($mode)] . '<br />' . sprintf($user->lang['RETURN_' . strtoupper($mode)], '<a href="' . $redirect_url . '">', '</a>');
				}
				else
				{
					$message = $user->lang['ERR_WATCHING'] . '<br />' . sprintf($user->lang['RETURN_' . strtoupper($mode)], '<a href="' . $redirect_url . '">', '</a>');
				}
