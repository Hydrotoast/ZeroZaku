...
// end
				$data = array(
					...
					'allow_all_comment'		=> request_var('allow_all_comment', (bool) $user->data['allow_all_comment']),
...
// end
						...
						$message = $user->lang['PREFERENCES_UPDATED'] . '<br />' . sprintf($user->lang['RETURN_UCP'], '<a href="' . $this->u_action . '">', '</a>');
				...
				$template->assign_vars(array(
					...
					'S_DST'				=> $data['dst'],
					'S_ALLOW_ALL_COMMENT'	=> $data['allow_all_comment'],
...
// end
			...
			case 'view':
				...
				if ($submit)
				{
					...
					if (!sizeof($error))
					{
						$user->optionset('mchat', $data['mchat']);
				...
				$template->assign_vars(array(
					'ERROR'				=> (sizeof($error)) ? implode('<br />', $error) : '',
					'MCHAT_ENABLE'		=> ($config['mchat_enable']) ? true : false,
