...
// end
			...
			case 'subscribed':
				...
				if ($unwatch)
				{
					...
					$message = $msg . '<br />' . sprintf($user->lang['RETURN_UCP'], '<a href="' . append_sid("{$phpbb_root_path}ucp.$phpEx", "i=$id&amp;mode=subscribed") . '">', '</a>');
			...
			case 'bookmarks':
				...
				if (isset($_POST['unbookmark']))
				{
					...
					if (confirm_box(true))
					{
						...
						meta_refresh(3, $url);
						$message = $user->lang['BOOKMARKS_REMOVED'] . '<br />' . sprintf($user->lang['RETURN_UCP'], '<a href="' . $url . '">', '</a>');
						trigger_error($message);
					}
					else
					{
				...
				if ($submit && $edit)
				{
					...
					if (check_form_key('ucp_draft'))
					{
						if ($draft_message && $draft_subject)
						{
							...
							$message = $user->lang['DRAFT_UPDATED'] . '<br />' . sprintf($user->lang['RETURN_UCP'], '<a href="' . $this->u_action . '">', '</a>');
