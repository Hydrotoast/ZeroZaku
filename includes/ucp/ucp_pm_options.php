...
function message_options($id, $mode, $global_privmsgs_rules, $global_rule_conditions)
{
	...
	if (isset($_POST['fullfolder']))
	{
		...
		if ($full_action)
		{
			...
			$message = $user->lang['FULL_FOLDER_OPTION_CHANGED'] . '<br />' . sprintf($user->lang['RETURN_UCP'], '<a href="' . $redirect_url . '">', '</a>');
	...
	if (isset($_POST['addfolder']))
	{
		...
		$message = $msg . '<br />' . sprintf($user->lang['RETURN_UCP'], '<a href="' . $redirect_url . '">', '</a>');
		meta_refresh(3, $redirect_url);
		trigger_error($message);
	}
	...
	if (isset($_POST['remove_folder']))
	{
		...
		if (confirm_box(true))
		{
			...
			$message .= '<br />' . sprintf($user->lang['RETURN_UCP'], '<a href="' . $meta_info . '">', '</a>');
			trigger_error($message);
		}
		else
		{
