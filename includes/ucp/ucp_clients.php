		...
		switch ($mode)
		{
			case 'front':
				...
				if ($submit && $edit)
				{
					...
					if (check_form_key('ucp_client'))
					{	
						if ($client_name && strlen($client_name) >= 3 && strlen($client_name) <= 24)
						{
							if($client_new_pass)
							{
								...
								if(strcmp($client_new_pass, $client_passconf) == 0 && phpbb_check_hash($client_passcur, $row['client_pass']) && strlen($client_new_pass) >= 3 && strlen($client_new_pass) <= 24)
								{
									...
									$message = $user->lang['CLIENT_UPDATED'] . '<br />' . sprintf($user->lang['RETURN_UCP'], '<a href="' . $this->u_action . '">', '</a>');
								...
								{
									$template->assign_var('ERROR', (strcmp($client_new_pass, $client_passconf) != 0 OR !phpbb_check_hash($client_passcur, $row['client_pass'])) ? $user->lang['PASS_NO_MATCH_OR_CUR'] : ((strlen($client_new_pass) < 3 OR strlen($client_new_pass) > 24) ? $user->lang['DISALLOWED_PASS_CHARS'] : ''));
								}
						...
						{
							$template->assign_var('ERROR', ($clien_name == '') ? $user->lang['EMPTY_CLIENT_NAME'] : (strlen($client_name) < 3 && strlen($client_name) > 24) ? $user->lang['DISALLOWED_NAME_CHARS'] : '');
						}
					}
					else
					{
				...
				foreach ($clientrows as $client)
				{
					...
					$template_row = array(
						...
						'U_CLIENT_PERMA'	=> append_sid("{$phpbb_root_path}clients.$phpEx", 'mode=password&amp;client_id=' . $client['client_id']),
			...
			case 'add':
				...
				if ($submit)
				{
					...
					{	
						if ($client_name && $client_pass && $client_passconf && strcmp($client_pass, $client_passconf) == 0 && strlen($client_name) >= 3 && strlen($client_name) <= 24 && strlen($client_pass) >= 3 && strlen($client_pass) <= 24)
						{
							...
							if($count <= 10)
							{
								...
								$message = $user->lang['CLIENT_ADDED'] . '<br />' . sprintf($user->lang['RETURN_UCP'], '<a href="' . $this->u_action . '">', '</a>');
						...
						{
							$template->assign_var('ERROR', ($client_name == '') ? $user->lang['EMPTY_CLIENT_NAME'] : (($client_pass == '') ? $user->lang['EMPTY_CLIENT_PASS'] : ((strcmp($client_pass, $client_passconf) != 0) ? $user->lang['PASS_NO_MATCH'] : ((strlen($client_name) < 3 OR strlen($client_name) > 24) ? $user->lang['DISALLOWED_NAME_CHARS'] : ((strlen($client_pass) < 3 OR strlen($client_pass) > 24) ? $user->lang['DISALLOWED_PASS_CHARS'] : '')))));
						}
					}
					else
					{
