...
class ucp_groups
{
	...
	function main($id, $mode)
	{
		...
		$return_page = '<br />' . sprintf($user->lang['RETURN_PAGE'], '<a href="' . $this->u_action . '">', '</a>');
		...
		switch ($mode)
		{
			...
			case 'manage':
				...
				if ($group_id)
				{
					...
					$avatar_img = (!empty($group_row['group_avatar'])) ? get_user_avatar($group_row['group_avatar'], $group_row['group_avatar_type'], $group_row['group_avatar_width'], $group_row['group_avatar_height'], 'GROUP_AVATAR') : '<img src="' . $phpbb_root_path . 'adm/images/no_avatar.png" alt="" />';
				...
				switch ($action)
				{
					...
					case 'approve':
						...
						trigger_error($user->lang['USERS_APPROVED'] . '<br />' . sprintf($user->lang['RETURN_PAGE'], '<a href="' . $this->u_action . '&amp;action=list&amp;g=' . $group_id . '">', '</a>'));
					...
					case 'default':
						...
						if (confirm_box(true))
						{
							...
							trigger_error($user->lang['GROUP_DEFS_UPDATED'] . '<br />' . sprintf($user->lang['RETURN_PAGE'], '<a href="' . $this->u_action . '&amp;action=list&amp;g=' . $group_id . '">', '</a>'));
						}
						else
						{
					...
					case 'deleteusers':
						...
						if (confirm_box(true))
						{
							...
							if ($error)
							{
								trigger_error($user->lang[$error] . '<br />' . sprintf($user->lang['RETURN_PAGE'], '<a href="' . $this->u_action . '&amp;action=list&amp;g=' . $group_id . '">', '</a>'));
							}

							trigger_error($user->lang['GROUP_USERS_REMOVE'] . '<br />' . sprintf($user->lang['RETURN_PAGE'], '<a href="' . $this->u_action . '&amp;action=list&amp;g=' . $group_id . '">', '</a>'));
						}
						else
						{
							confirm_box(false, $user->lang['CONFIRM_OPERATION'], build_hidden_fields(array(
					...
					case 'addusers':
						...
						if (confirm_box(true))
						{
							...
							trigger_error($user->lang['GROUP_USERS_ADDED'] . '<br />' . sprintf($user->lang['RETURN_PAGE'], '<a href="' . $this->u_action . '&amp;action=list&amp;g=' . $group_id . '">', '</a>'));
						}
						else
						{
							$s_hidden_fields = array(
						...
						trigger_error($user->lang['NO_USERS_ADDED'] . '<br />' . sprintf($user->lang['RETURN_PAGE'], '<a href="' . $this->u_action . '&amp;action=list&amp;g=' . $group_id . '">', '</a>'));
