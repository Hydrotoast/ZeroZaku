				...
				if ($submit)
				{
					...
					if (!sizeof($error))
					{
						...
						if (!empty($sql_ary['user_actkey']))
						{
							...
							$message = $user->lang[$message] . '<br />' . sprintf($user->lang['RETURN_INDEX'], '<a href="' . append_sid($phpbb_root_path . 'index.' . $phpEx) . '">', '</a>');
						...
						else
						{
							...
							$message = $user->lang[$message] . '<br />' . sprintf($user->lang['RETURN_UCP'], '<a href="' . $this->u_action . '">', '</a>');
						}
