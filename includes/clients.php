...
if(!in_array($mode, array('', 'client', 'groups', 'user', 'password')))
{
...
else
{
	...
	switch($mode)
	{
		...
		case 'groups':
			...
			break;
		case 'user':
			$sql = 'SELECT user_posts, user_warnings, group_id FROM ' . USERS_TABLE . ' WHERE user_id = ' . $user->data['user_id'];
			$result = $db->sql_query($sql);
			$row = $db->sql_fetchrow($result);
			...
			echo format_client_data($user_ary, $format);
