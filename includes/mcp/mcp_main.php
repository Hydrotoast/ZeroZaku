...
function lock_unlock($action, $ids)
{
	...
	else
	{
		...
		trigger_error($user->lang[$success_msg] . '<br />' . sprintf($user->lang['RETURN_PAGE'], '<a href="' . $redirect . '">', '</a>'));
	}
}
...
function mcp_move_topic($topic_ids)
{
	...
	else
	{
		...
		$message .= '<br />' . sprintf($user->lang['RETURN_PAGE'], '<a href="' . $redirect . '">', '</a>');
		$message .= '<br />' . sprintf($user->lang['RETURN_FORUM'], '<a href="' . append_sid("{$phpbb_root_path}viewforum.$phpEx", "f=$forum_id") . '">', '</a>');
		$message .= '<br />' . sprintf($user->lang['RETURN_NEW_FORUM'], '<a href="' . append_sid("{$phpbb_root_path}viewforum.$phpEx", "f=$to_forum_id") . '">', '</a>');
...
function mcp_delete_topic($topic_ids)
{
	...
	{
		meta_refresh(3, $redirect);
		trigger_error($user->lang[$success_msg] . '<br />' . sprintf($user->lang['RETURN_' . $redirect_message], '<a href="' . $redirect . '">', '</a>'));
	}
}
...
function mcp_delete_post($post_ids)
{
	...
	if (confirm_box(true))
	{
		...
		else
		{
			if ($deleted_topics)
			{
				...
				$success_msg = $user->lang['POSTS_DELETED_SUCCESS'] . '<br />' . $user->lang['EMPTY_TOPICS_REMOVED_WARNING'];
			}
			else
			{
	...
	else
	{
		...
		meta_refresh(3, $redirect);
		trigger_error($success_msg . '<br />' . sprintf($user->lang['RETURN_PAGE'], '<a href="' . $redirect . '">', '</a>') . '<br />' . implode('<br />', $return_link));
	}
}
...
function mcp_fork_topic($topic_ids)
{
	...
	else
	{
		...
		{
			$return_link .= '<br />' . sprintf($user->lang['RETURN_NEW_FORUM'], '<a href="' . append_sid("{$phpbb_root_path}viewforum.$phpEx", 'f=' . $to_forum_id) . '">', '</a>');
		}

		trigger_error($user->lang[$success_msg] . '<br />' . $return_link);
	}
}
