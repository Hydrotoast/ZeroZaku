...
if(!in_array($mchat_mode, array('', 'whois', 'clean', 'archive', 'read', 'stats', 'add', 'edit', 'delete')))
{
...
switch ($mchat_mode)
{
	...
	case 'whois';
		...
		{	
			...
			$template->assign_block_vars('navlinks', array(
				...
				'U_VIEW_FORUM' => append_sid("{$phpbb_root_path}mchat.$phpEx", 'mode=whois'),
			));
	...
	case 'clean';
		...
		else if ($mchat_founder)
		{
			if(confirm_box(true))
			{
				...
				trigger_error($user->lang['MCHAT_CLEANED'].'<br />'.sprintf($user->lang['RETURN_PAGE'], '<a href="'.$mchat_redirect.'">', '</a>'), E_USER_NOTICE);
			}
			else
			{
				// Display confirm box
	...
	case 'archive';
		...
		if (!$mchat_read_archive || !$mchat_view)
		{
			...
			meta_refresh(3, $mchat_redirect);
			trigger_error($user->lang['MCHAT_NOACCESS_ARCHIVE'].'<br />'.sprintf($user->lang['RETURN_PAGE'], '<a href="'.$mchat_redirect.'">', '</a>'), E_USER_NOTICE);
			// Stop running code
			exit;
		}
		...
		if ($config['mchat_enable'] && $mchat_read_archive && $mchat_view)
		{
			...
			foreach($rows as $row)
			{
				...
				$template->assign_block_vars('mchatrow', array(
					...
					'MCHAT_AVATAR'			=> get_user_avatar($row['user_avatar'], $row['user_avatar_type'], 40, 40),
	...
	case 'read':
		...
		$sql = 'SELECT m.*, u.username, u.user_colour, u.user_avatar, u.user_avatar_type, u.user_id as userid
	...
	default:
		...
		if (!$mchat_include_index)
		{
			...
			if (!$config_mchat['custom_page'] && $mchat_custom_page)
			{
				// Redirect to previous page
				meta_refresh(3, $mchat_redirect);
				trigger_error($user->lang['MCHAT_NO_CUSTOM_PAGE'].'<br />'.sprintf($user->lang['RETURN_PAGE'], '<a href="'.$mchat_redirect.'">', '</a>'), E_USER_NOTICE);
